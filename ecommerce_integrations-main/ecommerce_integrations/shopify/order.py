...
def auto_sync_new_and_old_orders():
    sync_new_orders()
    sync_old_orders()
...
def fetch_new_orders():
    # Logic to fetch new orders from Shopify
    pass
...
def create_sales_order(shopify_order, setting, company=None):
    ...
    if not so:
        ...
        so = frappe.get_doc(
            {
                ...
                "status": "Draft" if shopify_order.get("financial_status") == "pending" else "To Bill"
            }
        )
        ...
        if shopify_order.get("tags"):
            for tag in shopify_order.get("tags").split(","):
                so.add_comment(text=f"Order Tag: {tag.strip()}")
